  ...
  if (!date) {
    return (
      <div className="card-modern flex flex-col items-center justify-center text-gray-500 py-16 border-dashed border-2">
        <CalendarX2 size={48} className="text-slate-700 mb-4" />
        <p className="font-medium text-gray-400 text-lg">No date selected</p>
    ...
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-primary-light font-medium">{format(new Date(date), 'd MMMM yyyy')}</p>
        </div>
        <button onClick={() => { setSelected(null); setModal('create'); }} className="btn-primary flex items-center gap-2 shadow-lg shadow-primary/20">
      ...
      {loading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card-modern flex flex-col items-center justify-center text-center py-16 border-dashed border-2">
          ...
          <p className="font-medium text-gray-300 text-lg">No bookings yet</p>
      ...
      <Modal isOpen={modal === 'view'} onClose={() => setModal(null)} title="Booking Details" size="md">
        {selected && (
          <div className="space-y-4 text-sm">
            <div className="p-4 bg-slate-900/50 rounded-xl border border-dark-border grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1">Customer</span>
                <p className="text-white font-medium text-base">{selected.customerName}</p>
                <a href={`tel:${selected.mobile}`} className="text-primary-light hover:underline">{selected.mobile}</a>
              </div>
              <div>
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1">Slot</span>
                <p className="text-white">{selected.timeSlot}</p>
              </div>
              <div>
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1">Total Amount</span>
                <p className="text-white font-medium">₹{selected.totalAmount.toLocaleString('en-IN')}</p>
              </div>
            </div>
            
            {(selected.guestCount > 0 || selected.notes || selected.decorationNotes || selected.eventNotes) && (
              <div className="p-4 bg-slate-900/50 rounded-xl border border-dark-border space-y-4">
                {selected.guestCount > 0 && (
                  <div>
                    ...
                    <p className="text-gray-300">{selected.guestCount}</p>
                  </div>
                )}
                {selected.notes && (
                  <div>
